...
from matplotlib import pyplot as ppp
...
login_manager.login_view='/notfound/Unauthorized Entery'
#database import
...
def add_tracker():
    if request.method=='POST':
        ...
        se=request.form.get('settings')

        if n in [k.name for k in User.query.get(u).trackers]:
            return notfound('TRACKER NAME SHOULD BE UNIQUE')

        if type=='Multiple-choice':
            if set=="":
                return notfound('INVALID INPUT, MULTICHOICE SHOULD BE SEPARATED BY COMMAS.')
        elif set!="":
            set=""
        
        add=tracker(user_id=u,name=n,desc=d,type=ty,settings=se)
        try:
            data.session.add(add)
            data.session.commit()
            return main()
...
def view_tl(tracker_id):
    ...
    if (tracker_id,) not in data.session.query(tracker.tracker_id).all():
    ...
    ppp.plot(x,y,marker='o',color='b',linestyle='--')
    ppp.gcf().autofmt_xdate()
    ppp.savefig('static/chart.png')
    if len(x)>0:
    ...
    if request.method=='POST':
        try:
            if request.form.get('name')!=t.name or request.form.get('desc')!=t.desc or request.form.get('type')!=request.form.get('settings'):
                data.session.query(log).filter(log.tracker_id==tracker_id).delete()
            ...
            print(updict)
            data.session.query(tracker).filter(tracker.tracker_id==tracker_id).update(updict)
            data.session.commit()
            return main()
        except:
...
def update_log(log_id):#############more validation needed#######
    #validation
    if (log_id,) not in data.session.query(log.log_id).all():
    ...
    if request.method=='POST':
        ...
        log_datetime=datetime.strptime(request.form.get("time"),'%d/%b/%Y, %H:%M:%S.%f')
        #print(log_datetime)
        data.session.query(log).filter(log.log_id==log_id).update({'log_value':log_value,'note':log_note,'log_datetime':log_datetime})
        data.session.commit()
